...
#     logger.info('Creating vocabulary...')
    ...
    for fold in ('train', 'dev', 'test'):
        for instance in dataset.get_instances(fold):
            ...
            tokenized = tokenizer.tokenize(sentence, return_str=True)
            tokenized_list = tokenized.split()
            word_ids = [word_vocab[token] for token in tokenized_list if token in word_vocab]
            ret[fold].append(dict(word_ids=create_numpy_sequence(word_ids, max_word_length, np.int),label=instance.label))
...
def load_mldoc_dataset(dataset_path,lang,dev_size=0.05,seed=1):
    ...
    def read(mode,lang,instances):
        ...
        for category in categories:
            path = dataset_path + "/mldoc/" + lang + "/" + mode + "/" + category + ".txt"
